...
bool IsRoyalFlush(const array<unsigned short, 4>& hand){
    // A, K, Q, J, T = 4096 + 2048 + 1024 + 512 + 256 = 7936
    return 7936 == hand[0] || 7936 == hand[1] || 7936 == hand[2] || 7936 == hand[3];
}

bool DoesPlayer1Win(const array<unsigned short, 4>& player1Hand, const array<unsigned short, 4>& player2Hand){
...
void ProjectEuler054(){
    ...
    if (pokerFile.is_open()){
        while ( std::getline (pokerFile,line) ){
            ...
            if(DoesPlayer1Win(player1Hand, player2Hand)){
                player1WinCounter;
            }
